...
| **rtdetr-m.yaml** | 44,959,900 (~45.0M) | 170.5 | 515 | HGNetv2-M | Bản trung bình nằm giữa L và X |
...
Chúng tôi đã bổ sung **5 phiên bản RT-DETR mới** (`s`, `m`, `h`, `r18`, `r34`) bằng cách tối ưu hóa lại Backbone và cấu hình của Transformer Decoder để phân chia phân khúc mô hình tối ưu hơn cho các bài toán xử lý ảnh y tế (DentalYOLO).
...
2. **rtdetr-m (Medium):** Được tối ưu hóa cấu trúc nằm giữa L và X, sử dụng Stem (`[32, 56]`) và số block trung bình.
3. **rtdetr-h (Huge):** Sử dụng cấu hình Backbone HGNetv2-H cực lớn tham chiếu từ repo RT-DETRv2 PyTorch gốc, tăng số block xử lý sâu trên từng stage và sử dụng `hidden_dim = 512` cho chất lượng phát hiện tốt nhất.
